...
// Task bc delays
TASK(cell_sense, TASK_STACK_512) {
  ...
  while (true) {
    ...
    for (size_t i = 0; i < s_afe_settings.num_thermistors * s_afe_settings.num_devices; i += 2) {
      ...
      ltc_afe_storage->aux_voltages[ltc_afe_storage->aux_result_lookup[i]] = calculate_temperature(
    ...
    for (size_t thermistor = 0;
         thermistor < s_afe_settings.num_thermistors * s_afe_settings.num_devices;
         thermistor += 2) {
      // Log thermistor result
