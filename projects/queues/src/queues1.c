...
TASK(task1, TASK_STACK_512) {
  ...
  while (counter1 < 4) {
    // Your code goes here
    queue_send(&s_queue1, &s_list[counter1], 1000);
    delay_ms(100);
    if (queue_send(&s_queue1, &s_list[counter1], 1000) != STATUS_CODE_OK) {
...
TASK(task2, TASK_STACK_512) {
  ...
  while (true) {
    // Your code goes here
    queue_receive(&s_queue1, &received, 1000);
    delay_ms(100);
    if (queue_receive(&s_queue1, &received, 1000) == STATUS_CODE_OK) {
